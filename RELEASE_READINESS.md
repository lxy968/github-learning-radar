...
| Worker/队列/迁移可靠性 | 两类任务在持续积压时交替优先，空队列立即回退；健康摘要区分延迟重试与可领取任务，并对两类队列报告 stale、数量和等待超时原因。迁移使用跨进程 advisory lock、同 reserved 会话显式事务和逐文件 SHA-256；生产数据库 URL 强制 TLS 模式 | `7597a55` 真实 CI 暴露 reserved 连接没有 `sql.begin()`；现已改为显式 `BEGIN/COMMIT/ROLLBACK`。类型、成功提交/失败回滚逻辑、严格卫生、差异、生产构建和双模式 HTTP 回归通过；PostgreSQL 跨会话锁、实际校验和及会话清理竞态待修复提交 CI 确认 |
...
| GitHub 仓库设置 | Private 仓库已推送；发布候选 `7597a55` 的 `verify` 绿色、`container-integration` 红色，修复尚未推送；Dependabot 已创建升级 PR，但分支保护与安全设置尚未核对 | 修复提交完整 CI、分支保护、Required CI、Dependabot 绿色更新、Secret Scanning、Issue/PR 模板页面 | 先推送并确认修复 CI；再按 `RELEASE_CHECKLIST.md` 配置保护规则；红色 Dependabot PR 不合并，仓库继续保持 Private |
