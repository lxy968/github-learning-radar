...
| 会话、反馈与任务所有权收口 | 匿名令牌带签发时间并固定一年失效，Proxy 首次登记、后续只续活有效记录；清除/过期令牌不能重建原身份。过期清理按批执行并在 PostgreSQL 使用匿名 ID 过滤、跳锁和最终过期复核。Feedback 使用字段/媒体类型/字节/UTF-8 白名单，任务状态默认按当前会话所有权拒绝。项目已公开上线，首发直接重置旧格式本地开发 Cookie，不承担线上用户迁移 | 类型、逻辑、严格卫生、差异、生产构建和 full/showcase HTTP 回归通过；覆盖删除令牌重放、25 条跨批清理、反馈边界/异常流、跨会话与畸形任务 ID。PostgreSQL 续期锁竞态、四类子表级联和非匿名保护已加入 `container-integration`，远程实跑待提交后确认 |
...
| 完整 Git 历史与依赖安全 | 新命令枚举所有 refs 的历史 blob，拒绝历史环境文件、数据库、私钥、本地数据、构建目录和疑似 Token；输出不包含匹配内容。release check 与完整克隆 CI 均已接入。生产依赖审计发现 Next 传递的 PostCSS 中危 XSS 后，已定向覆盖到修复版 8.5.16 | 本地完整历史文本扫描通过；锁文件已固定 Next 使用 PostCSS 8.5.16，联网 `pnpm audit --prod --audit-level moderate` 返回 `No known vulnerabilities found`；类型、逻辑、严格卫生、差异、生产构建、线上 HTTP 与 CI #21 通过。公开仓库已启用 Secret Protection、Push Protection、Dependabot、Dependency Graph 和 Private Vulnerability Reporting |
...
| GitHub CI | 线上功能修复提交 `0a36840` 的 CI #21 与 README 收尾提交 `2f0b2f0` 的 CI #23 整体成功 | 通过；公开仓库可访问，CI #21 包含 Web/Worker 镜像、PostgreSQL 16 迁移与集成测试 |
...
| GitHub 仓库设置 | 已切换为 Public；未登录访问返回 `200`，README 与 clone URL 可见；默认分支为 `main`，CI #23 通过。Dependency Graph、Dependabot alerts/安全更新/恶意依赖告警、Private Vulnerability Reporting、Secret Protection 和 Push Protection 已启用 | Public 可访问、默认分支与 CI 正常，适用的依赖、漏洞和密钥防护已开启；严格分支保护尚未启用 | 如需 Required CI，配置不会阻碍个人维护的最小 Ruleset 后重新验证直接提交与 PR 流程 |
...
| 正式 Showcase 部署 | Vercel Hobby + Neon Free 已创建并部署，免费 HTTPS 域名为 `https://github-learning-radar-lxy968.vercel.app`；仓库公开后已设置 `PUBLIC_REPOSITORY_PUBLISHED=true` 并重新部署 | 新部署为 Ready；首页可访问，开源入口显示且指向 `https://github.com/lxy968/github-learning-radar`；原有迁移、健康、零 Key/零队列证据继续成立 | 通过；后续若改变运行模式、数据库或环境变量需重新验收 |
| 备份与回滚 | 没有真实数据库和镜像仓库 | 可恢复备份、镜像 digest 回滚、任务恢复和脱敏演练记录 | 预发布环境执行一次完整演练 |
| 正式发布资料 | 已有 Public 仓库、真实 Demo URL、浏览器截图、上线提交 `0a36840` 和 README 收尾提交 `2f0b2f0`；尚无 `v0.1.0` tag 和 GitHub Release | 最终发布截图、tag 和 Release | 确认发布文案后创建正式版本 |
...
6. Vercel Hobby + Neon Free 已完成最小权限部署、迁移、健康、公开仓库入口与线上回归；下一步补备份/回滚演练、最终截图、tag 和 Release。
