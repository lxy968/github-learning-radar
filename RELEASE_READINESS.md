...
| 仓库静态卫生 | Git for Windows 已安装并初始化 `main`；候选与忽略规则审计完成，非严格 `scripts/repository-hygiene.ts` 成功，仅因尚无首个提交跳过跟踪历史检查 | 条件通过 |
...
| --- | --- | --- | --- |
| Git 跟踪审计 | Git 2.55.0.windows.2 已安装，本地 `main` 已初始化；152 个候选文件约 1.05 MB，敏感环境/数据/构建目录均被忽略。尚无作者身份、index 和首个提交 | staged 文件清单、严格卫生检查、有效首个提交与提交后 clean status | 维护者提供 `user.name`/`user.email` 后，仅在本仓库配置身份并完成首个本地提交 |
...
| GitHub 仓库设置 | 尚无真实远端仓库 | 分支保护、Required CI、Dependabot、Secret Scanning、Issue/PR 模板页面 | 创建 GitHub 仓库并完成 `RELEASE_CHECKLIST.md` |
