...
| Git 跟踪审计 | Git 2.55.0.windows.2、本地 `main`、GitHub noreply 作者、首个提交 `7a188bd`；迁移修复提交 `3ef391e` 已推送到 Private 远端且本地/远端一致；严格仓库卫生与 `git diff --check` 通过 | 通过（本次未提交改动的 CI 待推送后确认） |
...
| Showcase 内置方案与进度闭环 | 当前推荐在 showcase 中获得同一仓库真实 DeepSeek Pro 缓存的 3/7/14 天完整方案；三个周期都有稳定 ID，不写方案表。内部 fixture 保留模型、调用轨迹和 Token 证据，公开接口会剥离这些元数据，只返回学习内容；步骤进度仍按匿名会话写入允许的进度存储 | 逻辑验证证明三个周期完整、ID 稳定、full 不注入；隔离生产 showcase HTTP 完成“读取方案 → PUT 第一步 → 再读相同 ID → GET 恢复完成态”，并确认 HTML 与公开 JSON 不含内部模型 ID。无 Worker、外部调用或新增 Token；真实 PostgreSQL 总行数/provider 指标待预发布复核 |
...
| 完整 Git 历史与依赖安全 | 新命令枚举所有 refs 的历史 blob，拒绝历史环境文件、数据库、私钥、本地数据、构建目录和疑似 Token；输出不包含匹配内容。release check 与完整克隆 CI 均已接入。生产依赖审计发现 Next 传递的 PostCSS 中危 XSS 后，已定向覆盖到修复版 8.5.16 | 本地 290 个历史文本 blob 路径扫描通过；锁文件已固定 Next 使用 PostCSS 8.5.16，联网 `pnpm audit --prod --audit-level moderate` 返回 `No known vulnerabilities found`；类型、逻辑、严格卫生、差异、生产构建和双模式 HTTP 回归通过。GitHub Secret Scanning 等远端能力仍待公开前授权配置 |
...
| GitHub CI | Private 仓库提交 `148d9ff` 的 CI #8 整体成功；发布候选 `7597a55` 的 `verify` 绿色，但 `container-integration` 因迁移 reserved 连接误用 `sql.begin()` 失败 | 本地修复已通过；修复提交推送后的 CI 待确认，当前不能标为发布绿色 |
...
| GitHub 仓库设置 | Private 仓库已推送到 `3ef391e`，本地与 `origin/main` 一致；Dependabot、分支保护和安全设置仍需在 GitHub 页面核对 | 本次发布改动的完整 CI、分支保护、Required CI、Dependabot、Secret Scanning、Issue/PR 模板页面 | 提交并推送本次改动，确认完整 CI 后再按 `RELEASE_CHECKLIST.md` 配置保护规则；验收完成前仓库继续保持 Private |
