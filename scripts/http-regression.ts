...
async function main() {
  ...
  for (const route of ["/candidates", "/library", "/routes", "/bookmarks", "/history", "/settings"]) {
  ...
  assert.match(learning.html, /缓存会同时校验仓库输入、学习水平、目标/);
  ...
  const healthPayload = JSON.parse(health.html) as { status?: string; storage?: string; taskQueue?: unknown };
