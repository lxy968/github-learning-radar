# GitHub 学习雷达 v0.1.0 发布说明（Release Candidate）

> 状态：GitHub 仓库与在线 Demo 已公开，尚未创建 Git tag 或 GitHub Release。
...
Showcase 已部署到 [github-learning-radar-lxy968.vercel.app](https://github-learning-radar-lxy968.vercel.app)，使用 Vercel Hobby + Neon Free 的零付费组合；运行时使用池化数据库 URL，迁移使用直连 URL。完整实例按需部署 Web、Worker 和受保护 Cron。详细步骤见 `DEPLOYMENT.md`。
...
- 完整 Git 历史文本扫描通过；生产依赖审计返回 `No known vulnerabilities found`。
- GitHub Actions CI #21 已通过 Web/Worker 镜像、PostgreSQL 16 迁移、集成测试和双模式回归，对应提交为 `0a36840`。
- README 收尾提交 `2f0b2f0` 的 GitHub Actions CI #23 已通过；公开仓库可在未登录状态下查看和克隆。
- 真实 Vercel URL 已走通首页、项目详情、3/7/14 天预置方案、完成一步和刷新恢复；Neon 健康状态为 `ok/postgres`，两类任务队列访问前后保持为零。多视口无控制台错误，真实键盘、屏幕阅读器和独立 Cookie 上下文仍未完成。
- Vercel Production 已将 `PUBLIC_REPOSITORY_PUBLISHED` 更新为 `true` 并完成重新部署；首页折叠说明中的开源入口已指向公开仓库。
- GitHub 已启用 Dependency Graph、Dependabot alerts、安全更新、恶意依赖告警、Private Vulnerability Reporting、Secret Protection 和 Push Protection。
...
- 完成 Neon 备份恢复和回滚演练。
- 补产品截图，并完成正式 `v0.1.0` tag 与 GitHub Release。
- 根据个人维护流程配置不会阻碍日常提交的 `main` 分支保护或 Ruleset；当前仍未启用 Required CI。
...
正式发布时复制本文作为 GitHub Release 内容，删除“草案”和未完成项，确认仓库与 Demo 链接，然后创建 `v0.1.0` tag。完整门禁见 `RELEASE_CHECKLIST.md`。
